...
def draw_keyboard_image(surface, rect, title_font, small_font):
    ...
    pygame.draw.polygon(
        surface,
        ACCENT,
        [(cx, y), (cx - 34, y + 70), (cx + 34, y + 70)],
    )
    pygame.draw.circle(surface, (110, 245, 255), (cx, y + 36), 7)
    surface.blit(title_font.render("Home Row Launch Position", True, TEXT_COLOR), (rect.x + 28, rect.y + 22))
    ...
    hint = "Feel the raised bumps on F and J. Keep eyes forward."
    surface.blit(small_font.render(hint, True, MUTED_TEXT), (rect.x + 235, rect.y + 225))
...
def run(screen, clock, base_dir):
    lesson_dir = Path(base_dir) / "lessons" / "lesson_1"
    intro_audio_path = lesson_dir / "lesson_1_intro.wav"
    intro_text = read_text(lesson_dir / "lesson_1_intro.txt")
    ...
    while True:
        ...
        prompt = "Press ␣ to launch Lesson 1"
...
if __name__ == "__main__":
    ...
    run(display, pygame.time.Clock(), Path(__file__).resolve().parents[2])
    pygame.quit()
